...
export default function PracticeStart({ params }: { params: { course: string } }) {
  ...
  if (loading) {return (<AuthGate><main style={{ padding: 24 }}>Đang tải đề…</main></AuthGate>)};
  if (err) {return (<AuthGate><main style={{ padding: 24, color: 'crimson' }}>Lỗi: {err}</main></AuthGate>)};
  if (questions.length === 0) return (<AuthGate><main style={{ padding: 24 }}>Chưa có câu hỏi cho môn {subject}.</main></AuthGate>);
